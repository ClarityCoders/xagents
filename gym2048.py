...
class Game2048Env(gym.Env):
    ...
    def step(self, action):
        ...
        return self.process_state(observation), reward, done, info
    ...
    def reset(self):
        ...
        return self.process_state(self.Matrix)
...
#             return True
    ...
    def process_state(self, state):
        state = state.flatten()
        # logx = np.zeros(state.shape)
        # state = np.log2(state, out=logx, where=state > 0)
